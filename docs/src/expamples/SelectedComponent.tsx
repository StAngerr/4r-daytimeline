    ...
    <div className="p-2 text-center bg-blue-50 border border-blue-200 rounded h-full flex items-center justify-center">
        <div className="flex items-center space-x-2">
            ...
            <div className="font-semibold text-sm text-blue-800">
                {selected.start.toLocaleTimeString([], {
                    hour: '2-digit',
                    minute: '2-digit',
                })}{' '}
                -{' '}
                {selected.end.toLocaleTimeString([], {
                    hour: '2-digit',
                    minute: '2-digit',
                })}
            </div>
...
const code = `
    ...
    const BookingDisplay = ({ selected }) => {
        const [isBooked, setIsBooked] = useState(false);
        const baseClass = 'px-3 py-1 text-xs rounded flex items-center space-x-1 transition-colors';
        const bookedClass =  isBooked 
                            ? 'bg-white text-emerald-600' 
                            : 'bg-emerald-700 text-white hover:bg-emerald-800';
        ...
        return (
            <div className="p-2 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded h-full flex flex-col items-center justify-center">
                ...
                <button 
                    className={baseClass + bookedClass}
                    onClick={(e) => {
                        e.stopPropagation();
                        setIsBooked(!isBooked);
                            ...
                            <div>
                                {selectedTime ? (
                                    ...
                                    <p className="font-medium">
                                        {getModeDescription(mode)}. Select a
                                        time slot to see the custom component.
                                    </p>
                                )}
                            </div>
